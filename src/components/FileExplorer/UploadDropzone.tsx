...
export function UploadDropzone({ currentFolderId, onUploadComplete }: UploadDropzoneProps) {
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: async (data, variables) => {
      ...
      // Force immediate network refetches so the UI shows the latest files right away
      try {
        await Promise.all([
          queryClient.refetchQueries({ queryKey: ['files', currentFolderId] }),
          queryClient.refetchQueries({ queryKey: ['files', null] }),
          queryClient.refetchQueries({ queryKey: ['files'] }),
        ])
      } catch (e) {
        // Swallow refetch errors to preserve existing success behavior and toasts
      }
      onUploadComplete()
